...
export default function TeamActivityFeed({ projectId }: TeamActivityFeedProps) {
    ...
    const parseMetadata = (metadata: any, activityType: string) => {
        if (!metadata) return null;

        switch (activityType) {
            case 'member_removed':
                if (metadata.removed_user_id) {
                    return `Removed a team member from the project`;
                }
                break;
            case 'member_promoted':
                if (metadata.promoted_user_id) {
                    return `Promoted a developer to status manager`;
                }
                break;
            case 'member_demoted':
                if (metadata.demoted_user_id) {
                    return `Removed status manager role from a developer`;
                }
                break;
            case 'status_changed':
                if (metadata.old_status && metadata.new_status) {
                    return `Changed project status from ${metadata.old_status} to ${metadata.new_status}`;
                }
                break;
            case 'application_accepted':
                return `Accepted a new team member application`;
            case 'application_rejected':
                return `Rejected a team member application`;
            default:
                return null;
        }
        return null;
                            ...
                            <div key={activity.id} className="p-4 hover:bg-gray-50">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {/* Activity Metadata */}
                                        {activity.metadata && parseMetadata(activity.metadata, activity.activity_type) && (
                                            <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                                <div className="text-sm text-blue-800">
                                                    {parseMetadata(activity.metadata, activity.activity_type)}
                                                </div>
                                            </div>
                                        )}
                                    </div>
